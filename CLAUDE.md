...
- **Serial commands are documented inline with `help`.** Every serial command must appear in the `help` output with a short description, in the **same change** that adds the command. Current commands include `dht`, `status`, `sync`, `help`, `show sms`, `drain sms`, `delete sms <i>`, `delete all sms`, `ntfy test`.
...
- **Secrets never go in markdown, examples, logs, or chat.** `secrets.h` is gitignored; `secrets.example.h` is the committed template with placeholder values. Copy it to `secrets.h` locally to build. This includes **ntfy topic URLs** — an ntfy topic name/URL is a bearer credential (anyone who knows it can publish to or subscribe to the channel; the user-facing channel here carries WiFi passwords on save). Real ntfy URLs go only in the gitignored `secrets.h`; `secrets.example.h`, `ConfigManager.cpp` fallback `#define`s, `.ino`/`.cpp` comments, and every `v8/docs/*.md` / `README.md` must use placeholders (e.g. `https://ntfy.sh/YOUR_NTFY_TOPIC`) or refer to the setting by name (`ntfyUrl`/`ntfyLogUrl`) — never the real host or topic slug. The same rule applies to any other externally-reachable identifier (webhook URLs, API tokens, channel/topic names) that would let a stranger reading the public repo interact with the device's live services.
