...
void MyWebServer::begin() {
    ...
    server.on("/data", HTTP_POST, std::bind(&MyWebServer::handlePostRequest, this));
...
void MyWebServer::handlePostRequest() {
    if (server.hasArg("plain")) {
        ...
        server.send(200, "application/json", "{\"received\":\"" + lastReceivedData + "\"}");
        sendDataToPythonServer(lastReceivedData);
    } else {
        server.send(400, "application/json", "{\"error\":\"No message body received\"}");
    }
}
...
String MyWebServer::getLastReceivedData() {
